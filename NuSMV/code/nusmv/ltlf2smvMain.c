...
static void ltlf2smv_print_module(const NuSMVEnv_ptr env,
                                 FILE* ostream, node_ptr);

static void print_usage(StreamMgr_ptr streams, int argc, char **argv) {
    StreamMgr_print_error(streams,  "%s: Converts an LTLF formula to a fragment of an "
            "SMV program.\n", argv[0]);
...
int main(int argc, char **argv)
{
  ...
  /* open and parse from the input file */
  {
    ...
    if ((char*)NULL == argv[i]) {
      StreamMgr_print_error(streams,  "Error: ltlf2smv : the input file is not specified.\n");
      exit(1);
    }
  ...
  {
    node_ptr out_smv_module = ltlf2smv_core(env,
                                           uniqueId,
                                           Compile_pop_distrib_ops(env, in_ltl_expr),
                                           single_fairness, NULL);

    ltlf2smv_print_module(env, output_file, out_smv_module);
