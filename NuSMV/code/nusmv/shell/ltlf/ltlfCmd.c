...
int CommandCheckLtlfSpec(NuSMVEnv_ptr env, int argc, char** argv);
int CommandGetLtlfUcore(NuSMVEnv_ptr env, int argc, char** argv);

/*---------------------------------------------------------------------------*/
/* Static function prototypes                                                */
/*---------------------------------------------------------------------------*/
static int UsageCheckLtlfSpec(const NuSMVEnv_ptr env);
static int UsageGetLtlfUcore(const NuSMVEnv_ptr env);
/*---------------------------------------------------------------------------*/
/* Definition of exported functions                                          */
/*---------------------------------------------------------------------------*/

void Ltlf_Init(NuSMVEnv_ptr env)
{
  // Cmd_CommandAdd(env, "check_ltlfspec", CommandCheckLtlfSpec, 0, false);
  Cmd_CommandAdd(env, "get_ltlf_ucore", CommandGetLtlfUcore, 0, false);
}
...
/*!
  ...
  <dl>
    ...
    <dt> <tt>-p "ltl-expr [IN context]"</tt>
       ...
       instance name which the variables in <tt>ltlf_expr</tt> must be
       evaluated in.
    <dt> <tt>-n number</tt>
...
int CommandCheckLtlfSpec(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  while ((c = util_getopt(argc,argv,"hmo:n:p:P:")) != EOF) {
    ...
    switch (c) {
    case 'h': return UsageCheckLtlfSpec(env);

    case 'n':
      if (formula != NIL(char)) return UsageCheckLtlfSpec(env);
      if (prop_no != -1) return UsageCheckLtlfSpec(env);
      if (formula_name != NIL(char)) return UsageCheckLtlfSpec(env);
    ...
    case 'o':
      if (useMore == 1) return UsageCheckLtlfSpec(env);
      dbgFileName = util_strsav(util_optarg);
      StreamMgr_print_output(streams,  "Output to file: %s\n", dbgFileName);
      break;

    case 'm':
      if (dbgFileName != NIL(char)) return UsageCheckLtlfSpec(env);
      useMore = 1;
      break;

    default:  return UsageCheckLtlfSpec(env);
    }
  }
  if (argc != util_optind) return UsageCheckLtlfSpec(env);
...
static int UsageCheckLtlfSpec(const NuSMVEnv_ptr env)
{
  ...
  StreamMgr_print_error(streams,  "usage: check_ltlfspec [-h] [-m | -o file] [-n number | -p \"ltlf_expr\" | -P \"name\"]\n");
...
int CommandGetLtlfUcore(NuSMVEnv_ptr env, int argc, char** argv)
{
  ...
  while ((c = util_getopt(argc,argv,"hso:mk:")) != EOF) {
    ...
    case 'm':
      if (dbgFileName != NIL(char)) return UsageGetLtlfUcore(env);
      useMore = 1;
      break;

    default:  return UsageGetLtlfUcore(env);
    }
  }
  if (argc != util_optind) return UsageGetLtlfUcore(env);
  ...
  if (useMore || (char*)NULL != dbgFileName) {
    if (OUTCOME_SUCCESS !=
        Cmd_Misc_open_pipe_or_file(env, dbgFileName, &outstream)) {
      status = 1; goto check_ltlfuc_exit;
    }
  }
