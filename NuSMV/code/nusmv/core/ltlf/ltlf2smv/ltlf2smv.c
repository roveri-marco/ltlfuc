/* ---------------------------------------------------------------------------
  ...
  This file is part of the ``ltlf2smv'' package of NuSMV version 2.
...
/*!
  ...
  see the header of file ltlf2smv.h
...
/*!
  ...
  \todo Missing description
*/
#define ENV_LTLF2SMV_DEFINE_DECL "l2sDefDecl"
...
/*!
  ...
  \todo Missing description
*/
#define ENV_LTLF2SMV_VAR_DECL "l2sVarDecl"
...
/*!
  ...
  \todo Missing description
*/
#define ENV_LTLF2SMV_TRANS_DECL "l2sTraDecl"
...
/*!
  ...
  \todo Missing description
*/
#define ENV_LTLF2SMV_JUSTICE_DECL "l2sJusDecl"
...
/*!
  ...
  \todo Missing description
*/
#define ENV_LTLF2SMV_INIT_DECL "l2sIniDecl"
...
/*!
  ...
  \todo Missing description
*/
#define ENV_LTLF2SMV_EXPR_TO_NAME "l2sE2N"
...
/*!
  ...
  \todo Missing description
*/
#define ENV_LTLF2SMV_UNIQUE_POS_NUMBER "l2sUPN"
...
/*!
  ...
  \todo Missing description
*/
#define ENV_LTLF2SMV_SPECIFICATION_NUMBER "l2sSN"
...
static node_ptr transform_ltlf_expression(const NuSMVEnv_ptr env,
                                         node_ptr t,
                                         boolean polarity,
                                         const Ltlf2SmvPrefixes* prefixes);
...
static void ltlf2smv_print_module(NuSMVEnv_ptr env, FILE* ostream, node_ptr module);
#endif
...
node_ptr ltlf2smv(NuSMVEnv_ptr env, unsigned int uniqueId,
                 node_ptr in_ltlf_expr)
{
  return ltlf2smv_core(env, uniqueId, in_ltlf_expr, false, NULL);
}

node_ptr ltlf2smv_single_justice(NuSMVEnv_ptr env, unsigned int uniqueId,
                                node_ptr in_ltlf_expr)
{
  return ltlf2smv_core(env, uniqueId, in_ltlf_expr, true, NULL);
}

node_ptr ltlf2smv_core(NuSMVEnv_ptr env,
                      ...
                      const Ltlf2SmvPrefixes* prefixes)
{
   ...
   STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  {
    ...
    nusmv_yylineno = node_get_lineno(in_ltlf_expr);
  }
  ...
  in_ltlf_expr = normalise_formula(env, in_ltlf_expr);
...
#ifdef OUTPUT_DEBUGGING
  ltlf2smv_print_module(env, stdout, in_ltlf_expr);
#endif
...
static node_ptr convert_ltlf2ltl_recur(const NuSMVEnv_ptr env,
  ...
  case WAREAD: case WAWRITE:
    result = perform_memory_sharing(env, ltlf);

    if (result != ltlf) {
      result = normalise_formula(env, result);
    }
...
/*!
  ...
  The specificationNumber is the same operand provided to ltlf2smv
                                      ...
                                      unsigned int specificationNumber)
{
  /* the body of the module being created during transformation.
     See transform_ltlf_expression for more info
  */
  ...
  NuSMVEnv_set_value(env, ENV_LTLF2SMV_DEFINE_DECL, define_declarations);
  NuSMVEnv_set_value(env, ENV_LTLF2SMV_TRANS_DECL, trans_declarations);
  NuSMVEnv_set_value(env, ENV_LTLF2SMV_INIT_DECL, init_declarations);
  NuSMVEnv_set_value(env, ENV_LTLF2SMV_JUSTICE_DECL, justice_declarations);
  NuSMVEnv_set_value(env, ENV_LTLF2SMV_VAR_DECL, var_declarations);

  NuSMVEnv_set_value(env, ENV_LTLF2SMV_EXPR_TO_NAME, expr_to_name_hash);

  /* Here 1 is added since 0 may clash with NULL value. */
  NuSMVEnv_set_or_replace_value(env, ENV_LTLF2SMV_UNIQUE_POS_NUMBER,
                                PTR_FROM_INT(void*, 1));

  NuSMVEnv_set_or_replace_value(env, ENV_LTLF2SMV_SPECIFICATION_NUMBER,
...
{
  NodeList_ptr define_declarations =
    NODE_LIST(NuSMVEnv_remove_value(env, ENV_LTLF2SMV_DEFINE_DECL));
  NodeList_ptr trans_declarations =
    NODE_LIST(NuSMVEnv_remove_value(env, ENV_LTLF2SMV_TRANS_DECL));
  NodeList_ptr var_declarations =
    NODE_LIST(NuSMVEnv_remove_value(env, ENV_LTLF2SMV_VAR_DECL));
  NodeList_ptr justice_declarations =
    NODE_LIST(NuSMVEnv_remove_value(env, ENV_LTLF2SMV_JUSTICE_DECL));
  NodeList_ptr init_declarations =
    NODE_LIST(NuSMVEnv_remove_value(env, ENV_LTLF2SMV_INIT_DECL));

  hash_ptr expr_to_name_hash =
    (hash_ptr)NuSMVEnv_remove_value(env, ENV_LTLF2SMV_EXPR_TO_NAME);
                                         ...
                                         const Ltlf2SmvPrefixes* prefixes)
{
  ...
  NodeList_ptr define_declarations =
    NODE_LIST(NuSMVEnv_get_value(env, ENV_LTLF2SMV_DEFINE_DECL));
  NodeList_ptr trans_declarations =
    NODE_LIST(NuSMVEnv_get_value(env, ENV_LTLF2SMV_TRANS_DECL));
  NodeList_ptr var_declarations =
    NODE_LIST(NuSMVEnv_get_value(env, ENV_LTLF2SMV_VAR_DECL));
  NodeList_ptr justice_declarations =
    NODE_LIST(NuSMVEnv_get_value(env, ENV_LTLF2SMV_JUSTICE_DECL));
  NodeList_ptr init_declarations =
    NODE_LIST(NuSMVEnv_get_value(env, ENV_LTLF2SMV_INIT_DECL));
  ...
  case OR:
    ...
    nameLeft = transform_ltlf_expression(env, car(t), polarity, prefixes);
    nameRight = transform_ltlf_expression(env, cdr(t), polarity, prefixes);
                             ...
                             const Ltlf2SmvPrefixes* prefixes)
{
  ...
  hash_ptr expr_to_name_hash =
    (hash_ptr)NuSMVEnv_get_value(env, ENV_LTLF2SMV_EXPR_TO_NAME);
...
/*!
  \brief After invocation of transform_ltlf_expression, this
                                    ...
                                    const Ltlf2SmvPrefixes* prefixes)
{
  ...
  unsigned int specificationNumber =
    (unsigned int)(nusmv_ptrint)NuSMVEnv_get_value(env, ENV_LTLF2SMV_SPECIFICATION_NUMBER) - 2;
  ...
  ltlf_module_base_name =
      ((NULL == prefixes || NULL == prefixes->ltlf_module_base_name)?
        LTLF_MODULE_BASE_NAME : prefixes->ltlf_module_base_name);
...
/*                           find_node(nodemgr, DEFINE, shared_defines, Nil), all_declr); */
  ...
  {
    string_ptr name = generate_string(strings, "%s%u", ltlf_module_base_name,
                                      specificationNumber);
                                   ...
                                   const char * prefix_name)
{
  ...
  unsigned int uniquePositiveNumber =
    (unsigned int)(nusmv_ptrint)NuSMVEnv_get_value(env, ENV_LTLF2SMV_UNIQUE_POS_NUMBER) - 1;
