...
#include "nusmv/core/ltlf/ltlf.h"
#include "nusmv/core/ltlf/ltlfInt.h"
#include "nusmv/core/ltlf/ltlf2smv/ltlf2smv.h"
...
/*!
  ...
  \todo Missing description
*/
typedef struct Ltlf_StructCheckLtlfSpec_TAG Ltlf_StructCheckLtlfSpec;

/*---------------------------------------------------------------------------*/
/* Structure declarations                                                    */
/*---------------------------------------------------------------------------*/

struct Ltlf_StructCheckLtlfSpec_TAG
{
  ...
  Ltlf_StructCheckLtlfSpec_oreg2smv oreg2smv; /* The tableau constructor
                                               ...
                                               LTL, that will be
                                               removed by ltl2smv */
  Ltlf_StructCheckLtlfSpec_ltl2smv ltl2smv;   /* The tableau constructor
...
static BddFsm_ptr
ltlPropAddTableau(Ltlf_StructCheckLtlfSpec_ptr, FlatHierarchy_ptr);
static void
ltlf_structcheckltlspec_remove_layer(Ltlf_StructCheckLtlfSpec_ptr);
static void ltlf_structcheckltlspec_finalize(Object_ptr object, void* dummy);
static void ltlf_structcheckltlspec_deinit(Ltlf_StructCheckLtlfSpec_ptr);
static void ltlf_structcheckltlspec_init(Ltlf_StructCheckLtlfSpec_ptr,
                                        NuSMVEnv_ptr env,
                                        Prop_ptr prop);
static void ltlf_structcheckltlspec_prepare(Ltlf_StructCheckLtlfSpec_ptr);
static int
ltlf_structcheckltlspec_build_tableau_and_prop_fsm(Ltlf_StructCheckLtlfSpec_ptr self);
static void
ltlf_structcheckltlspec_check_el_bwd(Ltlf_StructCheckLtlfSpec_ptr self);
static void
ltlf_structcheckltlspec_check_el_fwd(Ltlf_StructCheckLtlfSpec_ptr self);
static bdd_ptr ltlf_clean_bdd(Ltlf_StructCheckLtlfSpec_ptr, bdd_ptr);
							     ...
							     node_ptr names)
{
  ...
  res = ALLOC(Ltlf_StructCheckLtlfSpec, 1);
...
{
  LTLF_STRUCTCHECKLTLSPEC_CHECK_INSTANCE(self);
...
bdd_ptr Ltlf_StructCheckLtlfSpec_get_clean_s0(Ltlf_StructCheckLtlfSpec_ptr self)
{
  ...
  return ltlf_clean_bdd(self, self->s0);
}
...
void Ltlf_StructCheckLtlfSpec_build(Ltlf_StructCheckLtlfSpec_ptr self)
{
  ...
  CATCH(errmgr) {
    res = ltlf_structcheckltlspec_build_tableau_and_prop_fsm(self);
  }
  FAIL(errmgr) {
    ltlf_structcheckltlspec_remove_layer(self);
  ...
  if (res == 1) {
    ...
    StreamMgr_print_error(streams,
            "Ltlf_CheckLtlfSpec: Problems in Tableau generation.\n");
    ErrorMgr_nusmv_exit(errmgr, 1);
  }
}
...
void Ltlf_StructCheckLtlfSpec_check(Ltlf_StructCheckLtlfSpec_ptr self)
{
  ...
  if (full_fairness) {
    nusmv_assert(false);
    // ltlf_structcheckltlspec_check_compassion(self);
  } else {
    ...
    /* [VS] Making the switch here is probably ok for algorithms
       ...
       l2s it probably has to be moved up to Ltlf_CheckLtlfSpec (and
    ...
    case BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_EL_BWD:
      ltlf_structcheckltlspec_check_el_bwd(self);
      break;
    case BDD_OREG_JUSTICE_EMPTINESS_BDD_ALGORITHM_EL_FWD:
      ltlf_structcheckltlspec_check_el_fwd(self);
      break;
    default:
...
void Ltlf_StructCheckLtlfSpec_explain(Ltlf_StructCheckLtlfSpec_ptr self,
                                    NodeList_ptr symbols)
{
  const StreamMgr_ptr streams =
    STREAM_MGR(NuSMVEnv_get_value(EnvObject_get_environment(ENV_OBJECT(self)), ENV_STREAM_MANAGER));
  ...
  trace = Ltlf_StructCheckLtlfSpec_build_counter_example(self, symbols);
...
*/
static BddFsm_ptr ltlPropAddTableau(Ltlf_StructCheckLtlfSpec_ptr self,
...
void Ltlf_spec_to_hierarchy(NuSMVEnv_ptr env, Expr_ptr spec, node_ptr context,
                           ...
                           FlatHierarchy_ptr outfh)
{
  ...
  const OptsHandler_ptr opts =
    OPTS_HANDLER(NuSMVEnv_get_value(env, ENV_OPTS_HANDLER));
  const ExprMgr_ptr exprs = EXPR_MGR(NuSMVEnv_get_value(env, ENV_EXPR_MANAGER));
  unsigned int ltlf_spec_counter = 0;

  node_ptr module;
  char* module_name;
  FlatHierarchy_ptr modfh;
  int c;
  size_t module_name_len = strlen(LTLF_MODULE_BASE_NAME)+7;
  SymbTable_ptr st = FlatHierarchy_get_symb_table(outfh);

  nusmv_assert(! Wff_Rewrite_is_rewriting_needed(st, spec, context));

  if (NuSMVEnv_has_value(env, ENV_LTLF_SPEC_COUNTER)) {
    /* ALways put offset for avoiding clash between 0 and NULL */
    ltlf_spec_counter = NODE_TO_INT(NuSMVEnv_get_value(env, ENV_LTLF_SPEC_COUNTER)) - 2;
  }
  ...
  c = snprintf(module_name, module_name_len, "%s%u", LTLF_MODULE_BASE_NAME, ltlf_spec_counter);
  SNPRINTF_CHECK(c, module_name_len);

  /* Remember the offset of 2, and increment of 1 */
  NuSMVEnv_set_or_replace_value(env, ENV_LTLF_SPEC_COUNTER,
                                NODE_FROM_INT(ltlf_spec_counter + 3));

  /* constructs the module */
  module = what2smv(env, ltlf_spec_counter, spec);
...
ltlf_structcheckltlspec_build_tableau_and_prop_fsm(Ltlf_StructCheckLtlfSpec_ptr self)
{
  ...
  if (self->negate_formula) {
    ltlf_formula = ExprMgr_not(exprs, ltlf_formula);
  }

  /* time to construct the tableau of the (negated) formula */
  Ltlf_spec_to_hierarchy(EnvObject_get_environment(ENV_OBJECT(self)),
                        ltlf_formula, context,
                        self->oreg2smv,
                        self->tableau_layer,
                        hierarchy);
...
/*!
  ...
  Assumes the Ltlf_StructcCheckLtlfSpec structure being
  initialized before with Ltlf_StructCheckLtlfSpec_build.
*/
static void
ltlf_structcheckltlspec_check_compassion(Ltlf_StructCheckLtlfSpec_ptr self)
{
  LTLF_STRUCTCHECKLTLSPEC_CHECK_INSTANCE(self);
...
ltlf_structcheckltlspec_check_el_bwd(Ltlf_StructCheckLtlfSpec_ptr self)
{
  ...
  FAIL(errmgr) {
    ltlf_structcheckltlspec_remove_layer(self);
    StreamMgr_print_error(streams,
            "ltlf_checkltlspec_el_bwd: Problems in Tableau verification.\n");
    ErrorMgr_nusmv_exit(errmgr, 1);
    return;
  }
...
ltlf_structcheckltlspec_check_el_fwd(Ltlf_StructCheckLtlfSpec_ptr self)
{
                                       ...
                                       false));
  {
    ...
    FAIL(errmgr) {
      ltlf_structcheckltlspec_remove_layer(self);
      StreamMgr_print_error(streams,
              "ltlf_checkltlspec_el_fwd: Problems in Tableau verification.\n");
      ErrorMgr_nusmv_exit(errmgr, 1);
      return;
    }
  }
...
*/
static void ltlf_structcheckltlspec_remove_layer(Ltlf_StructCheckLtlfSpec_ptr self)
{
  BoolEnc_ptr bool_enc;
...
/*!
  ...
  \sa optional
*/
static bdd_ptr ltlf_clean_bdd(Ltlf_StructCheckLtlfSpec_ptr self, bdd_ptr bdd)
{
...
/*!
  ...
  \sa optional
*/
static void ltlf_structcheckltlspec_init(Ltlf_StructCheckLtlfSpec_ptr self,
                                        NuSMVEnv_ptr env,
                                        Prop_ptr prop)
{
  ...
  if (opt_ltl2smv_single_justice(opts)) {
    self->oreg2smv = ltlf2smv_single_justice;
  }
  else {
    self->oreg2smv = ltlf2smv;
  }
  self->ltl2smv = NULL;
  self->negate_formula = true;
  self->do_rewriting = true;

  ltlf_structcheckltlspec_prepare(self);
  ...
  OVERRIDE(Object, finalize) = ltlf_structcheckltlspec_finalize;
}
...
static void ltlf_structcheckltlspec_finalize(Object_ptr object, void* dummy)
{
  Ltlf_StructCheckLtlfSpec_ptr self = LTLF_STRUCTCHECKLTLSPEC(object);

  ltlf_structcheckltlspec_deinit(self);
...
/*!
  ...
  \sa optional
*/
static void ltlf_structcheckltlspec_deinit(Ltlf_StructCheckLtlfSpec_ptr self)
{
...
/*!
  ...
  \sa ltlf_structcheckltlspec_init
*/
static void ltlf_structcheckltlspec_prepare(Ltlf_StructCheckLtlfSpec_ptr self)
{
  const OptsHandler_ptr opts =
    OPTS_HANDLER(NuSMVEnv_get_value(EnvObject_get_environment(ENV_OBJECT(self)), ENV_OPTS_HANDLER));
  ...
  /* Calculate reachable states from the model fsm, if COI is not
     ...
     ltlf_tableau_forward_search is enabled, reachables will be
