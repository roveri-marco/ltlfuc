...
#           Response(n) = G ( x -> F (OR _{i=1} ^{n} (yi)))
#
# Usage: Response.pl n
#
...
if (@ARGV != 1) {
    die "Usage: Response.pl n\n\tproduces an n-part formula of the form Response(n) = G ( x -> F (OR _{i=1} ^{n} (yi)))\n\tUse flag -v for verbose, human-readable output.\n";
} #end if
...
$pattern = "G ( x -> F (y1"; #initialize the pattern
