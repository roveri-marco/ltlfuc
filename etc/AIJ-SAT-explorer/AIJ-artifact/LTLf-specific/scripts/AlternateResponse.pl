...
if (@ARGV != 1) {
    die "Usage: AlternateResponse.pl n\n\tproduces an n-part formula of the form AlternateResponse(n) = G ( x -> X ((~x) U (OR _{i=1} ^{n} (yi))))\n\tUse flag -v for verbose, human-readable output.\n";
} #end if
...
$pattern = "G ( x -> X ( (~x) U ( y1"; #initialize the pattern
