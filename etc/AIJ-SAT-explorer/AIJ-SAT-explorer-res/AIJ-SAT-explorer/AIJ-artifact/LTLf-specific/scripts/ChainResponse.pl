...
#           ChainResponse(n) = G ( x -> X (OR _{i=1} ^{n} (yi)))
#
# Usage: ChainResponse.pl n
#
...
if (@ARGV != 1) {
    die "Usage: ChainResponse.pl n\n\tproduces an n-part formula of the form ChainResponse(n) = G ( x -> X (OR _{i=1} ^{n} (yi)))\n\tUse flag -v for verbose, human-readable output.\n";
} #end if
