...
if (@ARGV != 1) {
    die "Usage: RespondedExistence.pl n\n\tproduces an n-part formula of the form RespondedExistence(n) = (F x) -> F (OR _{i=1} ^{n} (yi))\n\tUse flag -v for verbose, human-readable output.\n";
} #end if
...
$pattern = "(F x) -> (F (y1 "; #initialize the pattern
