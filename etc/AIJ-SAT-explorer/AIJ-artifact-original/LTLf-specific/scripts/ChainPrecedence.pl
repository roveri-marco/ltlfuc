...
#           ChainPrecedence(n) = G (( X x) -> (OR _{i=1} ^{n} (yi)))
#
# Usage: ChainPrecedence.pl n
#
...
if (@ARGV != 1) {
    die "Usage: ChainPrecedence.pl n\n\tproduces an n-part formula of the form ChainPrecedence(n) = G ( (X x) -> (OR _{i=1} ^{n} (yi)))\n\tUse flag -v for verbose, human-readable output.\n";
} #end if
...
$pattern = "G ( (X x) -> (y1"; #initialize the pattern
